...
- Integrate connection resolution with all other features available in this lib. For example,
  [filters](/docs/django/guide/filters), [ordering](/docs/django/guide/ordering) and
  [permissions](/docs/django/guide/permissions) can be used together with connections defined
...
`DjangoCursorConnection` requires a *strictly* ordered `QuerySet`, that is, no two entries in the `QuerySet`
...
[`@strawberry_django.order`](/docs/django/guide/ordering)) then cursors created by `DjangoCursorConnection` will not be compatible
