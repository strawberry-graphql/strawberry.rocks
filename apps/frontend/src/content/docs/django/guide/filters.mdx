...
> [!TIP]
> If you are using the [relay integration](/docs/django/guide/relay) and working with types inheriting
...
- see the [warning on `regex` / `iRegex`]() before exposing these on untrusted input
...
The previous version of filters can be enabled via [**USE\_DEPRECATED\_FILTERS**](/docs/django/guide/settings#strawberry_django)

> [!WARNING]
> If **USE\_DEPRECATED\_FILTERS** is not set to `True` legacy custom filtering
...
- enable **USE\_DEPRECATED\_FILTERS**
...
- disable **USE\_DEPRECATED\_FILTERS** - ***This is breaking change***