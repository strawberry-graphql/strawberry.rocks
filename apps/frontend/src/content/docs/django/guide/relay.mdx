...
- Integrate connection resolution with all other features available in this lib. For example,
  [filters](filters), [ordering](ordering) and
  [permissions](permissions) can be used together with connections defined
...
[`@strawberry_django.order`](./ordering)) then cursors created by `DjangoCursorConnection` will not be compatible
