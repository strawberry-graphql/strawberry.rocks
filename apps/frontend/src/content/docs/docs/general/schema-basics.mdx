...
[_resolver_](../types/resolvers) that provides some data to a field through a
function.
...
the [resolvers section](../types/resolvers).
...
[custom `StrawberryConfig` on the schema](../types/schema-configurations)
